...
Finally I chose LCT5/1 SiPM as sensor with lower dark current rate and better signal to noise ratio for high energy measurements.

\section{Study of the light yield}
...
I used $^{109}$Cd source to illuminate the rods. It's activity was $\sim 5.8 \mu Ci (215 kBq)$ while acquiring data.\\
...
All spectra analysis was performed with following ad hoc python tasks (discussed in ...):
\begin{itemize}
\item \mytokens{find_peak}: function that finds spectrum peaks
\item \mytokens{mg_fit}: multi-gaussian fit
\item \mytokens{fwhm_lin}: study of FWHM/$\sqrt{N}$ linearity 
\item \mytokens{cal_lin}: collected charge calibration
\item \mytokens{position_diff}: linearity of n-PE position
\item \mytokens{fit_area}: fitting integrated area or source photo-peak
\end{itemize}
An example of acquired spectrum with photo-peak is showed in Fig~\ref{fig:es_spec}.

\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.235]{imm/es_spec1.png}
\end{center}
...
Physical properties of photo-peak need to be extracted integrating the whole spectrum with a multi-gaussian fit using \mytokens{find_peak} and \mytokens{mg_fit} (Fig~\ref{fig:spec_int} and \ref{fig:spec_int_pk}).

\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.45]{imm/spec_int.png}
\end{center}
...
\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.45]{imm/spec_int_pk.png}
\end{center}
\caption{Integration of acquired spectrum of $^{109}Cd$ @ 22keV with EJ-260+ LCT5/1 (+3V). (Linear scale) } 
\label{fig:spec_int_pk}
\end{figure}

SiPM spectrum has Poissonian distribution (discussed in ..), and therefore I expected a constant value for $FWHM/\sqrt{N}$, where FWHM is the width of single PE peaks and N is the number of considered PE. Result example of \mytokens{fwhm_lin} is showed in Fig~\ref{fig:fwhm}.
...
Function (~\ref{eqn:cal}) is included in \mytokens{cal_lin} task. A calibration example is showed in Fig~\ref{fig:cal}.

\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.32]{imm/cal.png}
\end{center}
\caption{Calibration of integrated spectrum. [$^{109}Cd$ @ 22keV with EJ-260+ LCT5/1 (+3V)].} 
\label{fig:cal}
\end{figure}

I expected that difference between calibrated position of photon-electron peaks was $\sim 1e^-$. I considered this assumption as a further test for linearity (Fig~\ref{fig:diff}) and for this I used \mytokens{position_diff} task.
...
I obtained spectrum information integrating the area of single PE peaks. The resulting curve is the source photo-peak with a gaussian distribution (Fig~\ref{fig:peak}). I fitted this curve with \mytokens{fit_area} task, that is a simple gaussian fit which reads \emph{centroid} (C) position as collected charge, \emph{amplitude} as counting rate per second, and FWHM as resolution (R) defined by (\ref{eqn:res}).
...
\begin{figure}[!h]
\begin{center}
\includegraphics[scale=0.38]{imm/area.png}
\end{center}
\caption{Integrated photo-peak area. [$^{109}Cd$ @ 22keV with EJ-260+ LCT5/1 (+3V)].} 
\label{fig:peak}
\end{figure}

First PE width and amplitude values (1 to 5 $e^-$) are quite inconsistent compared to high energy peaks, because of high rate that causes peak distortion and consequently a difficulty to fit curves increasing errors.\\
